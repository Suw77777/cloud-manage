...
func handleVPC(action string, args []string) {
	...
	case "detail":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: cloud-cli vpc detail <vpc-id>\n")
			os.Exit(1)
		}
		vpcId := args[0]
	...
	case "vswitches":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: cloud-cli vpc vswitches <vpc-id>\n")
			os.Exit(1)
		}
		vpcId := args[0]
...
func handleSLB(action string, args []string) {
	...
	case "detail":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: cloud-cli slb detail <slb-id>\n")
			os.Exit(1)
		}
		slbId := args[0]
	...
	case "listeners":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: cloud-cli slb listeners <slb-id>\n")
			os.Exit(1)
		}
		slbId := args[0]
