...
func printUsage() {
	fmt.Println(`Cloud Manage CLI - 阿里云资源管理命令行工具
...
func handleECS(action string, args []string) {
	...
	case "detail":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: ecs detail <instance-id>\n")
			os.Exit(1)
		}
		instanceId := args[0]
	...
	case "start":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: ecs start <instance-id>\n")
			os.Exit(1)
		}
		instanceId := args[0]
	...
	case "stop":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: ecs stop <instance-id> [--force]\n")
			os.Exit(1)
		}
		instanceId := args[0]
		force := false
	...
	case "reboot":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: ecs reboot <instance-id> [--force]\n")
			os.Exit(1)
		}
		instanceId := args[0]
		force := false
...
func handleCMS(action string, args []string) {
	...
	case "metrics":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: cms metrics <instance-id>\n")
			os.Exit(1)
		}
		instanceId := args[0]
...
func handleSLS(action string, args []string) {
	...
	case "logstores":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: sls logstores <project>\n")
			os.Exit(1)
		}
		project := args[0]
	...
	case "logs":
		if len(args) < 2 {
			fmt.Fprintf(os.Stderr, "Usage: sls logs <project> <logstore> [--query <query>] [--from <timestamp>] [--to <timestamp>] [--max <lines>]\n")
			os.Exit(1)
		}
		project := args[0]
		logstore := args[1]
...
func handleOSS(action string, args []string) {
	...
	case "objects":
		if len(args) < 1 {
			fmt.Fprintf(os.Stderr, "Usage: oss objects <bucket> [--prefix <prefix>] [--max <count>]\n")
			os.Exit(1)
		}
		bucket := args[0]
		prefix := ""
