...
#### 决策 1: 凭证存储方式（v2 修订）
- **选择**: 可选保存（默认不保存，用户显式选择才保存）
- **理由**: 尊重不同用户的安全需求，服务器环境用环境变量，本地开发用配置文件
- **交互方式**: 混合方式（全局开关 `save_credentials` + 单次覆盖 `--save`）
...
#### 决策 3: 主密码缓存策略（v2 修订）
- **CLI 模式**: 每次都要输入密码（环境变量 `CLOUD_MASTER_PASSWORD` 覆盖，用于脚本）
- **GUI/TUI 模式**: 当前会话缓存（程序退出就忘记）
- **理由**: CLI 安全优先，GUI/TUI 便利性平衡
...
#### 决策 7: 参数优先级（v2 修订）
- **选择**: 命令行参数 > 环境变量 > `--profile` 指定的配置 > `current_profile` 的配置 > 默认值
- **理由**: 符合 Unix 惯例，profile 作为默认值可被覆盖
...
#### 决策 9: SLS 导出范围（v2 修订）
- **选择**: 只支持查询范围（时间范围 + 查询语句）
- **限制**: 大于 5000 条日志直接拒绝
- **理由**: "已选日志"功能复杂度高，收益低，暂不实现
...
#### 决策 11: 主密码复杂度
- **选择**: 默认复杂度要求（最少 8 位，大小写+数字），可通过配置文件修改
- **理由**: 安全性与灵活性平衡
...
#### 决策 13: 配置文件版本兼容
- **选择**: 版本号字段 + 自动迁移
- **理由**: 配置格式可能会演进，自动迁移提升用户体验

#### 决策 14: 配置文件初始化
- **选择**: 首次运行自动生成 demo + 手动 `config init` 生成
- **理由**: 双重保障，降低使用门槛

#### 决策 15: 配置文件注释（v2 修订）
- **选择**: 保留注释（使用 yaml.v3 的 Node 类型）
- **理由**: 注释本身就是最好的文档，不要相信用户会看外部文档
...
#### 决策 17: 错误处理分模式
- **选择**: CLI 显示技术细节，GUI/TUI 显示友好提示
- **理由**: 不同用户群有不同需求

#### 决策 18: 暗色主题（v2 修订）
- **选择**: 默认跟随系统，可手动切换，**持久化到配置文件**
- **理由**: 用户友好，配置集中

#### 决策 19: 并发查询
- **选择**: 默认并发数 3，可通过环境变量/配置文件修改
- **理由**: 平衡性能和资源占用

#### 决策 20: 内存限制
- **选择**: 默认 256MB，可通过环境变量/配置文件修改
- **理由**: 防止内存无限增长

#### 决策 21: GUI 界面风格
- **选择**: v0.2.0 微调统一风格，未来版本迁移到 Material Design
- **理由**: 分期实施，先保证功能

#### 决策 22: 虚拟滚动 vs 分页
- **选择**: 保持分页（当前实现）
- **理由**: SLS 日志场景分页足够，虚拟滚动收益不大

---

### 暂不实现的功能（记录备忘）

以下功能在 ROADMAP 中提到但决定暂不实现：
...
| GUI 加载动画 | 优先级低 | 是 |
| 响应式布局 | 桌面工具，固定窗口够用 | 否 |
| 懒加载 embed.FS | Go 限制，收益不大 | 否 |
| SLS 导出"已选日志" | 复杂度高，收益低 | 否 |
| 虚拟滚动 | 分页够用 | 否 |
