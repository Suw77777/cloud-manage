...
- 配置文件支持 (`~/.config/cloud-manage/config.yaml`)
- 多账号 Profile 存储（AES-256-GCM 加密存储 AK/SK）
- 主密码保护，每次启动输入解锁
- `config` 子命令管理凭证（init/add/remove/list/switch/show/reset）
- 参数优先级处理: 命令行 > 环境变量 > 配置文件 > 默认值
- 环境变量与配置文件不一致时打印警告
- 首次运行自动生成 demo 配置文件
- 每个配置项有中文注释
...
- 日志导出功能（CSV/JSON 格式）
- CLI `sls export` 子命令
- 支持时间格式混合 (ISO 8601, 相对时间如 1h/30m/7d, Unix 时间戳)
- 自动生成文件名 + `--output` 可覆盖
- 限制最多 5000 条

### 性能优化

- GC 调优: 默认 256MB 内存限制，可通过环境变量/配置文件修改
- 并发查询优化: 默认并发数 3，可通过环境变量/配置文件修改
- 前端产物压缩 (gzip/brotli)
- Go 编译优化: `-trimpath -ldflags "-s -w"`
- Provider 缓存机制

### 体验优化

- TUI 快捷键完善: 1-6 切换 Tab，vim + 方向键都支持
- 错误处理友好化: CLI 显示技术细节，GUI/TUI 显示友好提示
- 暗色主题: 跟随系统主题，可手动切换 (auto/light/dark)
